...
public class AbilityMongoRepository : IAbilityMongoRepository
{
    ...
    {
        var ability = await _abilitiesCollection.Find(a => a.Id == id).FirstOrDefaultAsync();
    ...
    public async Task<Result<List<Ability>>> GetByIdsAsync(List<string> ids)
    {
        ...
        {
           return Result<List<Ability>>.NotFound($"Can not find ability with id - {string.Join(',',ids)}");
        }
